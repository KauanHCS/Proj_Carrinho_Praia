...
function excluirProduto(id) {
    ...
    if (botao && !botao.classList.contains('confirmar-exclusao')) {
        ...
        // Botão já foi substituído pelo container
        
        // Criar botão de confirmar
        const botaoConfirmar = document.createElement('button');
        botaoConfirmar.className = 'btn btn-sm btn-danger confirmar-exclusao';
        botaoConfirmar.innerHTML = '<i class="bi bi-check-circle"></i> Confirmar';
...
// Adicionar função para calcular margem de lucro (se não existir)
function calcularMargemLucro() {
    const precoCompraProduto = document.getElementById('precoCompraProduto');
    const precoVendaProduto = document.getElementById('precoVendaProduto');
    const margemDisplay = document.getElementById('margemLucro'); // Se houver um elemento para mostrar a margem
    
    if (precoCompraProduto && precoVendaProduto) {
        const compra = parseFloat(precoCompraProduto.value) || 0;
        const venda = parseFloat(precoVendaProduto.value) || 0;
        
        if (compra > 0 && venda > 0) {
            const margem = ((venda - compra) / compra) * 100;
            console.log(`Margem de lucro: ${margem.toFixed(2)}%`);
            
            // Se houver um elemento para mostrar a margem, atualizar
            if (margemDisplay) {
                margemDisplay.textContent = `${margem.toFixed(2)}%`;
                margemDisplay.className = margem > 0 ? 'text-success' : 'text-danger';
            }
        }
    }
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Event listeners para calcular margem de lucro
    const precoCompraProduto = document.getElementById('precoCompraProduto');
    const precoVendaProduto = document.getElementById('precoVendaProduto');
    
    if (precoCompraProduto && precoVendaProduto) {
        precoCompraProduto.addEventListener('input', calcularMargemLucro);
        precoVendaProduto.addEventListener('input', calcularMargemLucro);
    }
});
